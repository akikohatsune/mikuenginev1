...
impl Attacks {
    pub const fn new() -> Self {
        ...
        Attacks { pawn_attacks, knight_attacks, king_attacks }
    }
}
...
fn rook_attacks_slow(sq: u32, occ: u64) -> u64 {
    ...
    // North
    let mut cr = r + 1;
    while cr <= 7 { let bit = 1u64 << (cr * 8 + f); attacks |= bit; if occ & bit != 0 { break; } cr += 1; }
    // South
    cr = r - 1;
    while cr >= 0 { let bit = 1u64 << (cr * 8 + f); attacks |= bit; if occ & bit != 0 { break; } cr -= 1; }
    // East
    let mut cf = f + 1;
    while cf <= 7 { let bit = 1u64 << (r * 8 + cf); attacks |= bit; if occ & bit != 0 { break; } cf += 1; }
    // West
    cf = f - 1;
    while cf >= 0 { let bit = 1u64 << (r * 8 + cf); attacks |= bit; if occ & bit != 0 { break; } cf -= 1; }
    attacks
}
...
pub fn init_magics() {
    MAGIC_TABLES.get_or_init(|| {
        ...
        for sq in 0..64u32 {
            ...
            rook_entries[sq as usize] = MagicEntry { mask, magic, shift, offset };
            ...
            for occ in subsets {
                ...
                if dest.0 == 0 || dest.0 == attacks {
                    *dest = Bitboard::new(attacks);
                    ...
                    debug_assert!(dest.0 == attacks, "destructive rook magic collision at sq={}", sq);
                }
            }
        ...
        for sq in 0..64u32 {
            ...
            bishop_entries[sq as usize] = MagicEntry { mask, magic, shift, offset };
            ...
            for occ in subsets {
                ...
                if dest.0 == 0 || dest.0 == attacks {
                    *dest = Bitboard::new(attacks);
                    ...
                    debug_assert!(dest.0 == attacks, "destructive bishop magic collision at sq={}", sq);
                }
            }
