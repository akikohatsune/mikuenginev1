...
impl Search {
    ...
    pub fn check_time(&mut self) {
        if self.nodes % 2048 == 0 {
            if self.thread_id == 0 && self.start_time.elapsed() >= self.hard_limit {
    ...
    //   QUIESCENCE SEARCH
    // ------------------------------------------------------------------
    pub fn quiescence(&mut self, board: &mut Board, mut alpha: i32, beta: i32, q_ply: usize) -> i32 {
        if q_ply > 200 {
            println!("q_ply max reached! stand_pat={}", board.nnue.evaluate(board.side_to_move, &board.accumulator));
            std::process::exit(1);
        }
        self.nodes += 1;
        self.check_time();
        ...
        while let Some(m) = picker.next(&self.heuristics, board) {
            ...
            if is_capture && !is_promo {
                ...
                if let Some(victim) = board.piece_on_sq[to_sq as usize] {
                    ...
                    if stand_pat + crate::eval::PAWN_VALUE + margin < alpha {
                        continue;
                    }
                }
            }
    ...
    // ------------------------------------------------------------------
    pub fn negamax(&mut self, board: &mut Board, mut depth: u8, mut alpha: i32, mut beta: i32, ply: usize, prev_move: Option<Move>, cut_node: bool) -> i32 {
        if ply > 200 {
            println!("max ply reached! depth={} in_check={} prev_move={:?} tt_hash={:x}", depth, board.is_square_attacked(Square::new((board.piece_bb(PieceType::King) & board.color_occupancy(board.side_to_move)).lsb()), board.side_to_move.flip()), prev_move, board.zobrist_key);
            std::process::exit(1);
        }
        
        let true_hash = board.compute_hash();
        if board.zobrist_key != true_hash {
            eprintln!("HASH DRIFT DETECTED IN NEGAMAX! incrementally={:x}, true={:x}, ply={}", board.zobrist_key, true_hash, ply);
            std::process::exit(1);
        }
        ...
        if board.halfmove_clock >= 100 { return draw_score; }
        if ply > 0 && board.is_repetition() { return draw_score; }
        ...
        if let Some(entry) = self.tt.probe(board.zobrist_key) {
            ...
            if !pv_node && entry.depth >= depth {
                if entry.node_type == NodeType::Exact { return entry.score; }
                if entry.node_type == NodeType::Alpha && entry.score <= alpha { return alpha; }
                if entry.node_type == NodeType::Beta && entry.score >= beta { return beta; }
            }
        ...
        let has_non_pawn_material = (board.colors[side as usize] & !(board.pieces[PieceType::Pawn as usize] | board.pieces[PieceType::King as usize])).is_not_empty();
        ...
        if !pv_node && !in_check && depth <= 6 {
            ...
            if opponent_worsening { rfp_margin -= depth as i32 * 25; }
        ...
        if cut_node && !in_check && depth >= 3 && has_non_pawn_material && eval >= beta {
            ...
            let mut r = 4 + depth as u8 / 3;
            if eval - beta > 0 {
                r += ((eval - beta) as u8 / 200).min(3);
            }
            if opponent_worsening { r = r.saturating_sub(1); }
            let null_depth = if depth > r { depth - r } else { 0 };
            let null_score = -self.negamax(board, null_depth, -beta, -beta + 1, ply + 1, None, !pv_node);
            ...
            if null_score >= beta {
                if depth >= 8 && ply < depth as usize {
                    let verify_score = self.negamax(board, null_depth, alpha, beta, ply + 1, None, false);
        ...
        let futility_margin = 75 + depth as i32 * 150;

        let countermove = if let Some(pm) = prev_move {
            if let Some(prev_piece) = board.piece_on_sq[pm.to_sq() as usize] {
                let cm = self.heuristics.get_countermove(prev_piece.piece_type(), Square::new(pm.to_sq()));
        ...
        while let Some(m) = picker.next(&self.heuristics, board) {
            ...
            if depth <= 4 && !is_capture && !is_promo && legal_moves > 0 {
                if !board.see_ge(m, -(depth as i32 * 80)) {
                    continue;
                }
            }
            ...
            if depth <= 6 && is_capture && !is_promo && legal_moves > 0 {
                ...
                let capt_hist = self.heuristics.get_capture_history(attacker_pt, Square::new(m.to_sq()), victim_pt);
            ...
            if board.zobrist_key != board.compute_hash() {
                eprintln!("HASH DRIFT AFTER MAKE_MOVE {:?}! incrementally={:x}, true={:x}, ply={}", m, board.zobrist_key, board.compute_hash(), ply);
                std::process::exit(1);
            }
            ...
            if move_is_legal {
                ...
                if !is_shuffling {
                    if let Some(ttm) = tt_move {
                        ...
                        board.unmake_move(m, &undo);
                        let se_score = self.negamax(board, se_depth, se_beta - 1, se_beta, ply, prev_move, cut_node);
                ...
                if attacker_pt == PieceType::Pawn && !is_capture {
                    let rank = m.to_sq() / 8;
                    if (side == crate::types::Color::White && rank >= 6) || (side == crate::types::Color::Black && rank <= 1) {
                ...
                if !is_capture && !is_promo {
                    stat_score = self.heuristics.get_history(side, attacker_pt, Square::new(m.to_sq()));
                    if attacker_pt == PieceType::Pawn {
                        stat_score += self.heuristics.get_pawn_history(side, Square::new(m.from_sq()), Square::new(m.to_sq()));
                    } else if attacker_pt == PieceType::Knight || attacker_pt == PieceType::Bishop {
                        stat_score += self.heuristics.get_minor_piece_history(side, Square::new(m.from_sq()), Square::new(m.to_sq()));
                    }
                    stat_score += self.heuristics.get_low_ply_history(m, ply);
                    
                    if let Some(pm) = prev_move {
                        if let Some(prev_piece) = board.piece_on_sq[pm.to_sq() as usize] {
                            stat_score += self.heuristics.get_continuation(prev_piece.piece_type(), Square::new(pm.to_sq()), attacker_pt, Square::new(m.to_sq()));
                        }
                    }
                }
                ...
                if !pv_node && is_capture && !is_promo && !gives_check && depth <= 5 {
                    ...
                    let capt_hist = self.heuristics.get_capture_history(attacker_pt, Square::new(m.to_sq()), victim_pt);
                    let futility_value = eval + 232 + 217 * (depth as i32) + crate::eval::PIECE_VALUES[victim_pt as usize] * 10 + capt_hist / 100;
                ...
                if legal_moves >= 3 && depth >= 3 && !is_capture && !is_promo && !in_check && !gives_check {
                    ...
                    if !improving { r_frac += 1024; }
                    
                    // #19 cutNode LMR boost
                    if cut_node { r_frac += 1024; }
                    
                    // #20 ttCapture LMR boost
                    if let Some(ttm) = tt_move {
                        if ttm.is_capture() { r_frac += 1024; }
                    }
                    
                    // ttPv LMR reduction reduction
                    if tt_is_pv { r_frac -= 1024; }
                    ...
                    let h = self.heuristics.get_history(side, attacker_pt, Square::new(m.to_sq()));
                    if h > 4000 { r_frac -= 1024; }
                    if h < -2000 { r_frac += 1024; }
                    
                    // statScore LMR (smooth fractional offset)
                    r_frac -= stat_score / 4;
                    
                    let mut r = (r_frac / 1024).max(0);
                    
                    let reduced_depth = (new_depth as i32 - r).max(1) as u8;
                    
                    score = -self.negamax(board, reduced_depth, -alpha - 1, -alpha, ply + 1, Some(m), true);
                ...
                if needs_full_search {
                    if legal_moves == 1 {
                        score = -self.negamax(board, new_depth, -beta, -alpha, ply + 1, Some(m), false);
                    } else {
                        score = -self.negamax(board, new_depth, -alpha - 1, -alpha, ply + 1, Some(m), !pv_node);
                    ...
                    if legal_moves >= 3 && score > alpha && !is_capture && !is_promo {
                        if let Some(pm) = prev_move {
                            if let Some(prev_piece) = board.piece_on_sq[pm.to_sq() as usize] {
                                ...
                                self.heuristics.update_continuation(prev_piece.piece_type(), Square::new(pm.to_sq()), attacker_pt, Square::new(m.to_sq()), depth);
                            }
                        }
                    }
                }
                ...
                if score >= beta {
                    ...
                    if !is_capture {
                        ...
                        if see_ok {
                            self.heuristics.update_history(side, attacker_pt, Square::new(m.to_sq()), depth);
                            if attacker_pt == PieceType::Pawn {
                                self.heuristics.update_pawn_history(side, Square::new(m.from_sq()), Square::new(m.to_sq()), depth);
                            } else if attacker_pt == PieceType::Knight || attacker_pt == PieceType::Bishop {
                                self.heuristics.update_minor_piece_history(side, Square::new(m.from_sq()), Square::new(m.to_sq()), depth);
                            }
                            self.heuristics.update_low_ply_history(m, ply, depth);
                        } else {
                            self.heuristics.penalize_history(side, attacker_pt, Square::new(m.to_sq()), depth);
                            if attacker_pt == PieceType::Pawn {
                                self.heuristics.penalize_pawn_history(side, Square::new(m.from_sq()), Square::new(m.to_sq()), depth);
                            } else if attacker_pt == PieceType::Knight || attacker_pt == PieceType::Bishop {
                                self.heuristics.penalize_minor_piece_history(side, Square::new(m.from_sq()), Square::new(m.to_sq()), depth);
                            }
                        ...
                        if let Some(pm) = prev_move {
                            if let Some(prev_piece) = board.piece_on_sq[pm.to_sq() as usize] {
                                self.heuristics.update_countermove(prev_piece.piece_type(), Square::new(pm.to_sq()), m);
                                if see_ok {
                                    self.heuristics.update_continuation(prev_piece.piece_type(), Square::new(pm.to_sq()), attacker_pt, Square::new(m.to_sq()), depth);
                                }
                            }
                        ...
                        for q in 0..quiet_count {
                            let qm = quiets_searched[q];
                            if qm.0 != m.0 {
                                if let Some(qp) = board.piece_on_sq[qm.from_sq() as usize] {
                                    self.heuristics.penalize_history(side, qp.piece_type(), Square::new(qm.to_sq()), depth);
                                    if qp.piece_type() == PieceType::Pawn {
                                        self.heuristics.penalize_pawn_history(side, Square::new(qm.from_sq()), Square::new(qm.to_sq()), depth);
                                    } else if qp.piece_type() == PieceType::Knight || qp.piece_type() == PieceType::Bishop {
                                        self.heuristics.penalize_minor_piece_history(side, Square::new(qm.from_sq()), Square::new(qm.to_sq()), depth);
                                    }
                        ...
                        let victim_pt = board.piece_on_sq[to_sq as usize]
                            .map(|p| p.piece_type())
                            .unwrap_or(PieceType::Pawn);
                        self.heuristics.update_capture_history(attacker_pt, Square::new(to_sq), victim_pt, depth);
                    }
                    ...
                    self.tt.store(board.zobrist_key, depth, ret_score, NodeType::Beta, m, pv_node);
                    return ret_score;
                }
                ...
                if !is_capture && !is_promo {
                    ...
                    if !pv_node && !in_check && depth <= 3 && best_score > -MATE_SCORE {
                        let fmc = if depth == 1 { 2 } else if depth == 2 { 4 } else { 8 };
                        if quiet_count >= fmc {
                            picker.skip_quiets();
                        }
                    }
        ...
        self.tt.store(board.zobrist_key, depth, alpha, node_type, best_m, pv_node);
    ...
    pub fn iterate(&mut self, board: &mut Board, max_depth: u8) -> Move {
        ...
        for d in 1..=max_depth {
            ...
            if d >= 5 {
                ...
                loop {
                    ...
                    let search_depth = if d > 1 { (d as i32 - failed_high_count - search_again_counter / 2).max(1) as u8 } else { 1 };
                    let s = self.negamax(board, search_depth, a, b, 0, None, false);
                    
                    if self.stop.load(Ordering::Relaxed) { return self.best_move; }
            ...
            if d >= 6 && average_score - score > 50 {
                if self.soft_limit != Duration::from_secs(u64::MAX) {
                    let extra = Duration::from_millis(
                        (self.soft_limit.as_millis() as u64).min(2000)
                    );
        ...
        if self.thread_id == 0 {
            println!("info depth {} nodes {} time {} nps {} hashfull {} score {} pv {}", d, self.nodes, elapsed, nps, hashfull, score_str, pv_str.trim());
        }
            ...
            if self.soft_limit != Duration::from_secs(u64::MAX) {
                ...
                effective_soft_limit = Duration::from_millis((self.soft_limit.as_millis() as f64 * instability_factor) as u64);
                ...
                if d > 5 {
                    ...
                    if effort > 0.7 {
                        ...
                        effective_soft_limit = Duration::from_millis((effective_soft_limit.as_millis() as f64 * scale.max(0.5)) as u64);
                    }
                }
            }
