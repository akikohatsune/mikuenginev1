    ...
    for line in stdin.lock().lines() {
        ...
        match cmd {
            ...
            "go" => {
                ...
                parse_go(board.clone(), tt.clone(), stop_flag.clone(), num_threads, &tokens);
            }
            "stop" => {
...
fn parse_position(board: &mut Board, tokens: &[&str], nnue: Arc<NNUE>) {
    ...
    if tokens.len() > moves_idx && tokens[moves_idx] == "moves" {
        for i in (moves_idx + 1)..tokens.len() {
            ...
            let from_file = m_str.chars().nth(0).unwrap() as u8 - b'a';
            ...
            for j in 0..list.count {
                ...
                if m.from_sq() == from_sq && m.to_sq() == to_sq {
                    ...
                    if m_str.len() == 5 {
                        if m_promo == promo_type && ((m.0 >> 12) & 0x3 != 0 || (board.piece_on_sq[from_sq as usize].unwrap().piece_type() == PieceType::Pawn && (to_rank == 7 || to_rank == 0))) {
                            board.make_move(m);
                            break;
                        }
                    } else {
...
fn parse_go(board: Board, tt: Arc<TranspositionTable>, stop: Arc<AtomicBool>, num_threads: usize, tokens: &[&str]) {
    let mut depth = 6;
    let mut wtime = 0;
    let mut btime = 0;
    let mut winc = 0;
    ...
    while i < tokens.len() {
        match tokens[i] {
            "depth" => { depth = tokens[i+1].parse().unwrap_or(6); i += 1; }
    ...
    if movetime > 0 {
        final_soft_limit = std::time::Duration::from_millis((movetime as u64).saturating_sub(50).max(1));
        final_hard_limit = std::time::Duration::from_millis((movetime as u64).saturating_sub(10).max(1));
        depth = 64;
    } else if (wtime > 0 && board.side_to_move == Color::White) || (btime > 0 && board.side_to_move == Color::Black) {
        ...
        final_hard_limit = std::time::Duration::from_millis((allocated * 3).min(time.saturating_sub(50)).max(10));
        depth = 64;
    }
