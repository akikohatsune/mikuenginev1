...
impl Heuristics {
    ...
    pub fn update_history(&mut self, color: Color, pt: PieceType, to: Square, depth: u8) {
        ...
        Self::gravity_update(&mut self.history[color as usize][pt as usize][to.0 as usize], bonus);
    }
    ...
    #[inline(always)]
    pub fn update_capture_history(&mut self, attacker: PieceType, to: Square, victim: PieceType, depth: u8) {
        let bonus = (depth as i32) * (depth as i32);
        Self::gravity_update(&mut self.capture_history[attacker as usize][to.0 as usize][victim as usize], bonus);
    }
    ...
    #[inline(always)]
    pub fn update_continuation(&mut self, prev_pt: PieceType, prev_to: Square, cur_pt: PieceType, cur_to: Square, depth: u8) {
        let bonus = (depth as i32) * (depth as i32);
        Self::gravity_update(&mut self.cont_history[prev_pt as usize][prev_to.0 as usize][cur_pt as usize][cur_to.0 as usize], bonus);
    }

    #[inline(always)]
    pub fn get_continuation(&self, prev_pt: PieceType, prev_to: Square, cur_pt: PieceType, cur_to: Square) -> i32 {
    ...
    pub fn killer_slot(&self, m: Move, ply: usize) -> usize {
        ...
        if self.killers[ply][0].0 == m.0 { return 1; }
        if self.killers[ply][1].0 == m.0 { return 2; }
        0
    }
    ...
    pub fn update_pawn_history(&mut self, color: Color, from: Square, to: Square, depth: u8) {
        let bonus = (depth as i32) * (depth as i32);
        Self::gravity_update(&mut self.pawn_history[color as usize][from.0 as usize][to.0 as usize], bonus);
    }
    ...
    pub fn penalize_pawn_history(&mut self, color: Color, from: Square, to: Square, depth: u8) {
        ...
        Self::gravity_update(&mut self.pawn_history[color as usize][from.0 as usize][to.0 as usize], penalty);
    }
    
    #[inline(always)]
    pub fn update_minor_piece_history(&mut self, color: Color, from: Square, to: Square, depth: u8) {
        let bonus = (depth as i32) * (depth as i32);
        Self::gravity_update(&mut self.minor_piece_history[color as usize][from.0 as usize][to.0 as usize], bonus);
    }
    ...
    #[inline(always)]
    pub fn penalize_minor_piece_history(&mut self, color: Color, from: Square, to: Square, depth: u8) {
        let penalty = -((depth as i32) * (depth as i32));
        Self::gravity_update(&mut self.minor_piece_history[color as usize][from.0 as usize][to.0 as usize], penalty);
    }
    ...
    pub fn update_low_ply_history(&mut self, m: Move, ply: usize, depth: u8) {
        if ply < 16 {
            let bonus = (depth as i32) * (depth as i32);
            Self::gravity_update(&mut self.low_ply_history[ply][(m.0 & 0xFFF) as usize], bonus);
        }
    }
    ...
    pub fn penalize_low_ply_history(&mut self, m: Move, ply: usize, depth: u8) {
        if ply < 16 {
            let penalty = -((depth as i32) * (depth as i32));
            Self::gravity_update(&mut self.low_ply_history[ply][(m.0 & 0xFFF) as usize], penalty);
        }
    }
